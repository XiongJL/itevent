...
layui.use(['form','layer','element','laydate'], function(){
    ...
    form.on('submit(sub)', function (data) {
        ...
        //console.log(data)
        $.ajax({
            url: "/itevent/newAssets?index="+index+"&event=3",
...
function addItem() {
    ...
    if (trNum>=limit){
        ...
        document.getElementById("item-table").insertAdjacentHTML("beforeend",' <tr>\n' +
            ...
            '                            </td>\n' +
            '                            <td>\n' +
            '                                <select id="brand'+itemIndex+'" name="brand'+itemIndex+'" lay-filter="brand"lay-verify="" ></select>\n' +
            '                            </td>\n' +
            '                            <td>\n' +
